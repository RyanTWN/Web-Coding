...
module.exports = function createNatureRouter({ pool, requireAuth, requireOwnSeat, getTaipeiDateKey, isDateKey }) {
...
// 讀取今日未完成題組、每日彙總學習日曆與尚未精熟的錯題。
  router.get('/nature-progress', requireAuth, requireOwnSeat, async (req, res) => {
...
// 儲存每次 20 題進度；同一天可完成多次，但未完成的同一次題組不可更換。
  router.post('/nature-progress', requireAuth, requireOwnSeat, async (req, res) => {
  const {
