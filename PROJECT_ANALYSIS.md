...
- **自然科學實驗室 (Nature Science)**：
  ...
  - 涵蓋天氣變化、地表變動、熱傳播、電與磁、簡單機械、能量、聲音、生物與永續環境等單元。
  - **每日 20 題分層架構**：**核心基礎 12 題**、**統整加深 6 題**（填空 Cloze、配對 Pair）、**素養挑戰 2 題**（時序 Sequence、生活情境/圖表 Scenario/Chart）。
  - **同輪嚴格去重機制**：題幹文字與 ID 雜湊過濾（`seenPrompts`），同輪 20 題絕對 0 重複，重測隨機重排。
  - **一分鐘快問快答（是非題模式）**：獨立 60 秒限時衝刺、35+ 題迷思觀念庫、Combo 連對計數與完整迷思破除檢討。
  - 支援**單日多回合重測 (Multiple Attempts)** 與**錯題本與掌握度機制 (`nature_wrong_questions`)**。
- **社會人文探索館 (Social Humanities)**：
  ...
  - **同輪嚴格去重機制**：題幹文字與 ID 雜湊過濾（`seenPrompts`），同輪 20 題絕對 0 重複，重測隨機重排。
  - **一分鐘快問快答（是非題模式）**：獨立 60 秒限時衝刺、35+ 題迷思觀念庫、Combo 連對計數與完整迷思破除檢討。
  - 支援**單日多回合重測 (Multiple Attempts)**、**錯題本與熟練掌握度機制 (`social_wrong_questions`)** 與月度學習日曆打卡。
- **管理員後台 (Admin Portal)**：
  - 學生名冊管理、座號啟用與密碼重設。
