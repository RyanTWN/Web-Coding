...
- **英語天地 (English)**：
  ...
  - 3D 漫畫風格翻轉單字卡（含音標、發音語音合成 TTS、中文釋義、例句與翻譯），全面相容 iOS Safari WebKit 透視景深與圖層穿透。
  - 多型態隨堂測驗（單字選擇題、拼字填空題、聽力測驗題），支援星號收藏難字專用拼字測驗模式。
  - 難字星號標記、**星號單字鎖定保護模式**（防日常隨機滾動覆蓋個人重點清單）與學習歷程日曆。
  - 頂部「返回學科大廳」快速導航，並已具備**莫蘭迪微光玻璃 (Glassmorphism) 字卡視覺原型** (`preview_glass_flashcard.html`)。
- **數學天地 (Math)**：
  - 涵蓋國小六年級核心單元（最大公因數與公倍數、分數四則、小數除法、比與比值、圓與扇形、速率、柱體表面積/體積、基準量與比較量、怎樣解題、比例尺、統計圖）。
  - 110 組多樣化獨立題型產生器，導入**題型洗牌分派與題幹文字/數值特徵指紋雙重防重池**，確保同一輪測驗 100% 絕對零重複。
  ...
  - 頂部整合「返回學科大廳」快速導航與學習日曆數據可視化。
- **自然科學實驗室 (Nature Science)**：
  - 115 學年度課綱，支援三大主流教科書版本：**康軒、南一、翰林**（精準對齊康軒六上第 2 單元「水溶液」溶解、導電性與酸鹼中和）。
  - **每日 20 題分層架構**：**核心基礎 12 題**、**統整加深 6 題**（填空 Cloze、配對 Pair）、**素養挑戰 2 題**（時序 Sequence、生活情境/圖表 Scenario/Chart）。
  - **同輪全域嚴格去重機制**：題幹文字與 ID 雜湊過濾（`seenPrompts`），排除跨難度重複，同輪 20 題絕對 0 重複，重測隨機重排。
  - **題號格式全面相容**：完美支援包含底線識別碼（如 `kangxuan_ch2_q1`）之題號，消除答題推進卡關隱患。
  - **一分鐘快問快答（是非題模式）**：獨立 60 秒限時衝刺、45+ 題迷思觀念庫、Combo 連對計數與完整迷思破除檢討。
  - 支援**單日多回合重測 (Multiple Attempts)** 與**錯題本掌握度機制 (`nature_wrong_questions`)**，頂部具備直通大廳導航。
- **社會人文探索館 (Social Humanities)**：
  - 115 學年度課綱，支援三大主流教科書版本：**康軒、南一、翰林**完整六上與六下章節。
  - 涵蓋公民法治、臺灣現代化發展、生活經濟、世界地理、全球多元文化、國際組織與永續島嶼等 8 大核心主題（官方教科書標準題庫）。
  - **每日 20 題分層架構**：**核心基礎 12 題**、**統整加深 6 題**（填空 Cloze、配對 Pair）、**素養挑戰 2 題**（時序 Sequence、生活情境/圖表 Scenario/Chart）。
  - **同輪全域嚴格去重機制**：題幹文字與 ID 雜湊過濾（`seenPrompts`），同輪 20 題絕對 0 重複，重測隨機重排。
  - **題號格式全面相容**：完整相容含底線之題號 ID，答題推進與動畫流暢無阻。
  - **一分鐘快問快答（是非題模式）**：獨立 60 秒限時衝刺、35+ 題迷思觀念庫、Combo 連對計數與完整迷思破除檢討。
  - 支援**單日多回合重測 (Multiple Attempts)**、**錯題本與熟練掌握度機制 (`social_wrong_questions`)** 與月度學習日曆打卡，頂部具備直通大廳導航。
- **國語文天地與成語200探索館 (Chinese & Idioms 200)**：
  - 115 學年度課綱，支援三大主流教科書版本：**康軒、南一、翰林**完整六上與六下章節。
  - 涵蓋心靈自我、土地鄉情、自然感悟、多元閱讀、寓言智慧、迎向挑戰、藝術美學等 10 大核心主題。
  - **每日 20 題分層架構**：核心基礎 12 題、統整加深 6 題、素養挑戰 2 題，同輪絕對 0 重複，支援多回合重測與錯題複習機制 (`chinese_wrong_questions`)。
  - **成語 200 探索館**：完整收錄臺南市成語競賽 200 則核心題庫，教育部標準注音、字詞解析、近反義詞與生活例句。
  - **3D 擬真字卡翻轉學習**：支援 TTS 語音發音朗讀、星號難詞收藏 (`student_idiom_stars`)、關鍵字即時搜尋、鍵盤快捷翻卡。
  - **獨立成語隨堂測驗評量**：包含釋義選成語、成語選釋義、情境造句克漏字填空，自動組卷評分。
- **家長與管理員模組 (Guardian & Admin Portals)**：
  - **家長端跨科學習同步**：即時整合子女在英文、數學、自然、社會、國語五大學科的每日學習進度、測驗回合次數與各科累積錯題數。
  - **管理員後台**：學生名冊管理、座號啟用與密碼重設；修復學生歷程詳細彈窗與欄位呈現；單字庫 CRUD 與學習開關 (`learning_enabled`)；班級排行榜數據分析。
...
| `preview_glass_flashcard.html` | 視覺原型：莫蘭迪微光玻璃質感字卡、TTS 發音與 3D 翻轉展示 |
...
| **Phase 7: 社會人文學科全面啟用與 115 課綱題庫** | 2026-09-04 | • 比照自然科架構，完整上線「社會人文探索館」（康軒/南一/翰林三大版本）<br>• 打造 8 大核心主題事實庫（80+ 組核心題庫）、每日 20 題與單日多回合 (`attempt_no`)<br>• 建立 `social_daily_progress` 與 `social_wrong_questions` 錯題精熟掌握機制<br>• 整合月度學習日曆統計與首頁大廳解鎖導向<br>• 打造 `social_logic.test.js` 單元測試套件，達成 6 大測試套件 100% PASS | 已完成 |
| **Phase 8: 自然與社會 12+6+2 題型架構升級與「一分鐘快問快答」模式** | 2026-09-04 | • **每日 20 題全面分層重構**：嚴格落實**核心基礎 12 題 ＋ 統整加深 6 題（Cloze/Pair） ＋ 素養挑戰 2 題（Sequence/Scenario/Chart）**。<br>• **同輪測驗絕對去重保證**：建立題幹文字與 ID 雜湊篩選（`seenPrompts` Set），同輪 20 題絕對 0 重複，重測隨機打散選項洗牌。<br>• **獨立「一分鐘快問快答」限時是非題挑戰**：60 秒倒數計時進度條、35+ 題迷思觀念庫、Combo 連對激勵、本地最高紀錄榜與完整檢討報告。<br>• **自然與社會雙科同步升級**：`nature.js`/`nature.html` 與 `social.js`/`social.html` 全面實作上線。<br>• **自動化驗證與 NAS 正式上線**：單元測試 100% PASS，透過 `nas-auto-deploy` 經由 GitHub Actions CI/CD 建置與 NAS 內部主動部署驗證上線。 | 已完成 |
| **Phase 9: 康軒 115 課綱自然單元精準對齊與題庫擴充** | 2026-09-04 | • **章節目次精準核對**：依康軒官方 115 學年度課綱，將六上第 2 單元由原先誤植的「熱對物質的影響」修正為「水溶液」（活動1: 溶解與質量守恆、活動2: 水溶液導電性、活動3: 酸鹼性與酸鹼中和）。<br>• **真實題庫融入**：完整收錄小六自然隨堂練習單（第1回~第5回）：包含天氣變化、雲霧霜露、水循環、颱風路徑、氣候變遷與碳/水足跡、溶液質量守恆、電解質導電性、指示劑顏色變化等真實考題。<br>• **題庫架構擴增**：新增 `solutions` 核心主題（16 題核心題）、統整加深題（6 題）、素養挑戰題（3 題），並大幅擴充 45+ 題是非題庫與迷思破除詳解。<br>• **自動化測試與 NAS 線上驗證**：擴充 `nature_logic.test.js` 嚴格斷言單元對照與題庫深度，全數通過後經 GitHub Actions CI/CD 與 NAS 部署上線。 | 已完成 |
| **Phase 10: 家長與管理員數據同步、後台彈窗修復與星號拼字測驗回歸** | 2026-09-04 | • **家長端跨科學習同步**：後端 `routes/guardian.js` 擴充子女各科學習數據，完整串接英文、數學、自然、社會四大學科的當日進度、測驗回合與累積錯題統計。<br>• **管理員後台修復**：`admin.js` 與 `admin.html` 修復學生歷程詳細彈窗無法開啟與欄位遺漏問題，強化班級排行展示。<br>• **星號拼字測驗恢復**：`quiz.js` 與 `app.js` 重整拼字測驗流程，完整支援星號收藏難字的自訂抽考模式。 | 已完成 |
| **Phase 11: 星號單字專用鎖定模式、學科大廳導航與 iOS 翻牌佈局最佳化** | 2026-09-05 | • **星號單字鎖定保護**：`english.html` 與 `app.js` 實作星號鎖定機制，避免日常 30 字循環滾動覆蓋個人星號重點加強清單。<br>• **全學科大廳導航**：於英文、數學、自然、社會各頁頂部加入「返回學科大廳」導航按鈕，解決行動裝置返回困難問題。<br>• **iOS 翻牌相容性修復**：解決 iOS WebKit/Safari 在 3D 翻轉卡片時的 `backface-visibility`、透視景深與圖層穿透問題，翻牌體驗更加滑順。 | 已完成 |
| **Phase 12: 題號底線識別相容性與題庫推進流程最佳化** | 2026-09-06 | • **底線題號支援**：後端 `routes/nature.js`、`routes/social.js` 與前端 `nature.js`、`social.js` 全面相容帶有底線的題目 ID（例如 `kangxuan_ch2_q1`），消弭答題至特定題號卡關無法進入下一題的隱患。<br>• **推進流程優化**：答題回饋動畫與題序前進狀態機整合，並新增單元測試 `tests/social_logic.test.js` 確保底線格式通過。 | 已完成 |
| **Phase 13: 數理社三學科每日測驗同輪防重演算法強化** | 2026-09-06 | • **數學科題型與數值防重池**：`math.js` 重構隨機抽樣邏輯，建立歷史題型與數值特徵指紋池，確保同輪 10/20 題絕對不出現同類型題型或雷同數值。<br>• **自然與社會全域題幹去重**：`nature.js` 與 `social.js` 在 12 基礎 + 6 加深 + 2 素養的配比架構下，加入題幹文字與主鍵全域檢核，避免跨題型類別抽到相同題目。<br>• **全端測試通過**：後端 6 大測試套件 100% 通過，同輪絕對 0 重複。 | 已完成 |
| **Phase 14: 莫蘭迪微光玻璃擬物化 (Glassmorphism) 字卡視覺探索** | 2026-09-13 | • **全新視覺原型**：產出 `cool_learning/preview_glass_flashcard.html` 獨立預覽頁面，採用莫蘭迪冰川藍與溫暖米白雙色系、毛玻璃微反光、質感立體陰影與字體層次排版。<br>• **互動與音效架構就緒**：支援單字發音 TTS、星號切換、例句摺疊展開與 3D 翻轉動態，作為後續英文學習頁視覺現代化升級依據。 | **預覽就緒** |
| **Phase 15: 數學天地「簡單、挑戰、素養」三等級分層評量系統升級** | 2026-09-23 | • **11 大核心單元全數三等級擴充**：全庫擴增至 143 組產生器（每單元 5 簡單基礎 + 5 挑戰進階 + 3 情境素養），全面收錄生活長文本與大眾運輸、理財比價、綠能減碳等情境題。<br>• **嚴格 5:3:2 分層抽題演算法**：每日 10 題依「5 簡單 + 3 挑戰 + 2 素養」由淺入深推進，同輪題型與數值指紋 0 重複。<br>• **前端視覺與向度反饋**：題目區塊整合三色動態等級徽章（綠/橘/紫）與情境框；結算畫面新增三大能力向度掌握度（簡單/挑戰/素養答對率）即時診斷。<br>• **全端測試通過**：`math_logic.test.js` 與後端 6 大測試套件 100% 通過。 | 已完成 |
...
| | 3D 字卡 / 發音 / 星號收藏 | 100% | 翻轉順暢，iOS Safari WebKit 渲染問題已修復 |
| | 星號單字鎖定模式與拼字測驗 | 100% | 鎖定防日常滾動覆蓋，支援星號字自訂拼字測驗 |
| | 拼字 / 聽力 / 選擇測驗 | 100% | 計分與提交至 `quiz_logs` 正常 |
| | 莫蘭迪玻璃質感字卡原型 | 100% | `preview_glass_flashcard.html` 獨立展示就緒 |
| **自然科學** | 三大版本切換 (康軒/南一/翰林) | 100% | 115 學年度課綱，水溶液與天氣變化題庫完整 |
| | 每日 20 題 12基礎+6加深+2素養架構 | 100% | 嚴格 12+6+2 配比，同輪全域題幹 0 重複，支援多回合 |
| | 一分鐘快問快答（是非題模式） | 100% | 60 秒限時、45+ 是非題庫、連對 Combo 與迷思檢討 |
| | 題號底線格式支援與流暢推進 | 100% | 完整支援 `_` 格式題號，無卡關問題 |
| | 錯題本與掌握度標記 | 100% | 錯題收錄與再次答對自動掌握 |
| **社會人文** | 三大版本切換 (康軒/南一/翰林) | 100% | 115學年度課綱，8大核心主題對應 |
| | 每日 20 題 12基礎+6加深+2素養架構 | 100% | 嚴格 12+6+2 配比，同輪全域題幹 0 重複，支援多回合 |
| | 一分鐘快問快答（是非題模式） | 100% | 60 秒限時、35+ 是非題庫、連對 Combo 與迷思檢討 |
| | 題號底線格式支援與流暢推進 | 100% | 完整支援 `_` 格式題號，無卡關問題 |
| | 錯題本與熟練掌握機制 | 100% | 專屬 `social_wrong_questions` 表，複習答對自動精熟 |
| | 學習日曆打卡與月度統計 | 100% | 每日 20 題完成點亮，累積題數與分數加總 |
| **數學天地** | 課綱題庫與 143 組多樣產生器 | 100% | 涵蓋小六 11 大核心單元，每單元 13 組獨立題型（簡單5/挑戰5/素養3） |
| | 5:3:2 嚴格分層抽題與防重去重 | 100% | 每日 10 題採 5 簡單 + 3 挑戰 + 2 素養，同輪 0 重複由淺入深 |
| | 獨立切換模式 (綜合/打底/挑戰/特訓) | 100% | 支援每日綜合 (5:3:2)、純基礎打底 (全簡單)、純進階挑戰 (全挑戰)、純素養特訓 (全素養) 四大模式 |
| | 三等級視覺徽章與向度掌握分析 | 100% | 前端三色動態標籤、情境文本卡片與結算畫面能力向度診斷 |
...
| | 家長端子女跨科學習統計即時同步 | 100% | 同步呈現英、數、自、社學習進度與錯題統計 |
| | 管理員後台學生詳情與排行 | 100% | 修正學生詳細彈窗與班級排行榜數據呈現 |
| | Google Sign-In & Apple OAuth | 90% | 後端驗證代碼就緒，待填入線上 Client ID |
| | 最小權限資料庫存取 | 100% | 腳本與 `.env` 配置規範已完成 |
| **導航與操作** | 全學科大廳快速導航列 | 100% | 各學科頂部直通首頁大廳，優化平板/手機動線 |
...
4. **評估莫蘭迪微光玻璃 (Glassmorphism) 視覺升級**：
   根據 `cool_learning/preview_glass_flashcard.html` 之設計成果，評估逐步將莫蘭迪冰川藍與米白色系、高質感微光玻璃擬物化風格套用至正式 `english.html` 單字卡與各學科練習介面，進一步提升長時間專注學習的視覺舒適度與現代品牌質感。
